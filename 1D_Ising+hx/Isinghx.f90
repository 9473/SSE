...
!-------------------------!
 use configuration;implicit none
 integer :: v1,v2,v3,v4,v,ir,i,output,a
...
if(zz==-1) then
 if(mod(opstring(i),4)==3) then
    opstring(v1/4)=opstring(v1/4)-1
 elseif (mod(opstring(i),4)==2) then
    opstring(v1/4)=opstring(v1/4)+1
 endif
!  a=ieor(mod(opstring(i),4),1)
!  opstring(i) = opstring(i)-mod(opstring(i),4)+a
endif
